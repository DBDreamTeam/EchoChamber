...
CREATE TABLE `albums`(
    ...
    `Privacy`   enum('Friends', 'Circles', 'FriendsOfFriends') NOT NULL DEFAULT 'Friends', /* Default added */
    PRIMARY KEY (`AlbumID`)
);

CREATE TABLE `comments`(
    `CommentID` int(10) AUTO_INCREMENT NOT NULL,
    `Time`      datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP, /* added default to current timestamp */
...
CREATE TABLE `message`(
    ...
    `DateTime`  datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, /* added DEFAULT CURRENT_TIMESTAMP and removed length */s
