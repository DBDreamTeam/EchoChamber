...
if ($conn->connect_error) {
	...
	$FreindshipTableSql = "SELECT * FROM friendships WHERE Username ='$CheckFriend'";
...
<h3><?php echo $CheckFriend; ?>'s profile </h3>
    ...
    <p><?php echo $CheckFriend; ?>'s friends: </p>
    <?php while($row = $FreindshipResult->fetch_assoc()) { ?>
	<p> </p>
	<?php echo $row['friends_array']; ?>
    <?php 
	} ?>
   ...
   <!-- Check if the logged user is friend with he/she yet-->
	...
	$FreindshipQuery = "SELECT * FROM friendships WHERE Username ='$user' AND friends_array = '$CheckFriend'";
	...
	<?php if(mysqli_num_rows($isFriend)>=1){	
		...
		<form method="POST">
			...
			if(isset($_POST['removeFriend'])) {
				$removeFriendQuery = "DELETE FROM friendships WHERE Username = '$user' AND friends_array = '$CheckFriend'";
	
				$removeFriend2 = "DELETE FROM friendships WHERE friends_array = '$user' AND Username = '$CheckFriend'";
